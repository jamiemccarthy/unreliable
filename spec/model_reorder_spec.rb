...
RSpec.describe "reorder" do
  it "does not randomize when reordered to PK" do
    expect(Cat.order(:name).reorder(:id).to_sql).to end_with(adapter_text('ORDER BY "cats"."id" ASC'))
  end
  ...
  it "randomizes when reordered to non-PK column" do
    expect(Cat.reorder(:name).to_sql).to end_with(adapter_text('ORDER BY "cats"."name" ASC, RANDOM()'))
  end
end
