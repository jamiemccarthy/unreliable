...
RSpec.describe "first and last" do
  it "does not randomize the query equivalent to .first" do
    expect(Cat.order(:id).limit(1).to_sql).to end_with(adapter_text('ORDER BY "cats"."id" ASC LIMIT 1'))
  end

  it "does not randomize the query equivalent to .last" do
    expect(Cat.order(id: :desc).limit(1).to_sql).to end_with(adapter_text('ORDER BY "cats"."id" DESC LIMIT 1'))
  end
